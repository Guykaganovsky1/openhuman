...
**Routing** (`AppRoutes.tsx`, HashRouter): `/` (Welcome), `/auth`, `/onboarding/*`, `/chat/:threadId?`, `/human`, `/brain`, `/connections`, `/flows` (+ `/flows/:id`, `/flows/draft`), `/invites`, `/notifications`, `/rewards`, `/settings/*`, `/feedback`, `/workflows` (the legacy SKILL.md hub — renders `Activity`, it is NOT a redirect) and `/workflows/run`. Back-compat redirects: `/home`→`/chat`, `/skills`→`/connections`, `/channels`→`/connections?tab=messaging`, `/intelligence` & `/activity`→`/settings/notifications`, `/routines`→`/flows`, `/flows/discoveries`→`/flows?view=discoveries`, `/webhooks`→`/settings/integrations`, which itself forwards to `/connections` (there is no webhooks UI to land on, and the route emits no `#webhooks` fragment). `/settings/automations` is the settings-side slug and also lands on `/flows`. No `/login`, `/mnemonic`, `/agents`, `/conversations`.
