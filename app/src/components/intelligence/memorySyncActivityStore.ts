...
/**
 * The row ids the Sources screen last listed. A stage event names its row three
 * ways (bare id, scoped id, legacy connection id) and only a candidate that
 * matches a real row can key the store the way the row reads it — so the list
 * has to be here, not only in the screen that fetched it.
 */
let knownSourceIds: ReadonlySet<string> = new Set();
...
export function applyStageEvent(data: SyncStageEventDetail | null | undefined): void {
  ...
  // The core also emits a scoped source_id (`workspace:folder:src_…`) whose bare
  // tail is the row id, so resolution goes through the ids this store is already
  // tracking: a raw prefixed id never matched the row that was added, and the
  // terminal `delete` missed, leaving the row stuck on "Syncing…".
  const rowId = resolveSyncRowId(data ?? null, knownSourceIds);
  if (!rowId) return;
...
/**
 * The Sources screen listed these rows.
 *
 * Two jobs. The ids are what a stage event's scoped `source_id` is resolved
 * against, and a live id the refreshed list no longer names is dropped: no
 * later event can clear it (nothing renders that row any more), so leaving it
 * would strand a permanent "Syncing…" on the row when it comes back (RC#5,
 * #3295).
 */
export function noteKnownSourceIds(ids: Iterable<string>): void {
  const live = new Set(ids);
  knownSourceIds = live;
  const staleSyncing = Array.from(state.syncingIds).filter(id => !live.has(id));
  const staleProgress = Array.from(state.progress.keys()).filter(id => !live.has(id));
  if (staleSyncing.length === 0 && staleProgress.length === 0) return;
  console.debug(`[ui-flow][memory-sync] reconcile: dropping stale syncing ids ${staleSyncing}`);
  const syncingIds = new Set(state.syncingIds);
  for (const id of staleSyncing) syncingIds.delete(id);
  const progress = new Map(state.progress);
  for (const id of staleProgress) progress.delete(id);
  for (const id of staleSyncing) liveSince.delete(id);
  commit({ syncingIds, progress, results: state.results });
}
