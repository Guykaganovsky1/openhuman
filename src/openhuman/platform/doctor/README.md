...
- Inspect daemon state file: JSON validity, heartbeat freshness (stale > 30s = error), scheduler component health (stale > 120s), and per-channel component freshness (stale > 300s). An **absent** state file is reported `Ok` ("not supervised by a daemon") — nothing in the product writes that file, so its absence is the normal state of an install rather than a fault; a file that exists but cannot be read or parsed is still an Error.
